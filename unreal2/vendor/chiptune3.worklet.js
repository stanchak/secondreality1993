...
class MPT extends AudioWorkletProcessor {
	...
	play(buffer, paused = false) {
		...
		// openmpt_module_create_from_memory COPIES the file data, so the staging
		// buffer is dead the moment it returns. Leaving it allocated leaked the
		// whole module (~100 KB of .s3m) into the wasm heap on every era switch.
	...
	stop() {
		...
		// play() allocates leftPtr/rightPtr; this used to free leftBufferPtr/
		// rightBufferPtr, which never exist — so the guards saw `undefined != 0`,
		// called _free(undefined) (a no-op free(NULL)) and left the real render
		// buffers allocated on every stop. play() calls stop() first, so each
		// module switch leaked a fresh pair.
		if (this.leftPtr) {
